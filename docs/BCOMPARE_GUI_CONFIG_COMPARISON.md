...
**Reproducibility caveat:** the screenshots were taken into a session scratchpad that has
since been cleared, and are not committed to this repository. The Beyond Compare columns
below are therefore transcriptions that cannot be re-derived from the repo alone; re-run
the inspection against a local Beyond Compare install to verify them. The teczka columns
cite source files and are checkable directly.
...
| **Edit** (9 items) | **Edit** (9 items) | BC's Edit is *selection* (Expand/Collapse All, Select All, Select All Files, Select Newer ▸, Select Orphans ▸, Invert Selection, Refresh, Full Refresh). teczka's Edit is *editing* (Undo, Redo, Copy L→R, Copy R→L, Swap Sides, Find, Find Next/Prev). Same name, disjoint content. |
...
| — | **Difference** (10 items) | teczka-only: Previous/Next File, Previous/Next Difference, Apply/Unapply Difference, Apply/Unapply All, Statistics. BC scatters these across Search and Actions. |
